...
tinyengine_status convolve_s8_kernel2x3_inputch3_stride2_pad1(const q7_t *input, const uint16_t input_x, const uint16_t input_y, const uint16_t input_ch, const q7_t *kernel,
	...
	for (int16_t i_out_y = 0; i_out_y < output_y; i_out_y++) {
		for (int16_t i_out_x = 0; i_out_x < output_x; i_out_x++) {
			...
			if (two_column_buf == runtime_buf + 2 * 18) {
				out = mat_mult_unloop18_s8_s16(kernel, runtime_buf, output_ch, output_shift, output_mult, output_offset,
