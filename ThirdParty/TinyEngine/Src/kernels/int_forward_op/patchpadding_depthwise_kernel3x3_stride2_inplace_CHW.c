...
void patch_depthwise_kernel3x3_stride2_inplace_kernel_CHW(const uint16_t output_y, const uint16_t output_x,
...
tinyengine_status patchpadding_depthwise_kernel3x3_stride2_inplace_CHW(q7_t *input, const uint16_t input_x, const uint16_t input_y, const uint16_t input_ch, const q7_t *kernel,
	...
	for (uint16_t i = pad_t; i < input_y - pad_b; i++) {
		// handle left padding
		for (uint16_t j = 0; j < pad_l; j++) {
			*cols_8b++ = PAD8;
		}
		cols_8b += input_x - (pad_l + pad_r);

		// handle right padding
		for (uint16_t j = 0; j < pad_r; j++) {
			*cols_8b++ = PAD8;
		}
	}
	...
	for (uint16_t c = 0; c < input_ch; c++) {
		...
		patch_depthwise_kernel3x3_stride2_inplace_kernel_CHW(output_y, output_x, bias++, biasR++, ksrc, output_mult++,
															 output_shift++, inplace_out, output_offset,
															 output_activation_min, output_activation_max,
															 cols_8b_start, input_x, input_ch);
		inplace_out++;
		input++;
		ksrc += 9;
	}
