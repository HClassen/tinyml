	...
	for (int i_batch = 0; i_batch < input_batches; i_batch++) {
		...
		for (int16_t i_out_y = 0; i_out_y < output_y; i_out_y++) {
			for (int16_t i_out_x = 0; i_out_x < output_x; i_out_x++) {
				for (int16_t i_ker_y = i_out_y * stride_y - pad_y; i_ker_y < i_out_y * stride_y - pad_y + kernel_y; i_ker_y++) {
					for (int16_t i_ker_x = i_out_x * stride_x - pad_x; i_ker_x < i_out_x * stride_x - pad_x + kernel_x; i_ker_x++) {
						if (i_ker_y < 0 || i_ker_y >= input_y || i_ker_x < 0 || i_ker_x >= input_x) {
							/* Filling 0 for out-of-bound paddings */
							memset(four_column_buf, 0, sizeof(q15_t) * input_ch);
						} else {
							/* Copying the pixel data to column */
							arm_q7_to_q15_with_offset(input + (i_ker_y * input_x + i_ker_x) * input_ch, four_column_buf,
													  input_ch, input_offset);
						}
				...
				if (four_column_buf == buffer_a + 4 * input_ch * kernel_y * kernel_x) {
					out = arm_nn_mat_mult_kernel_s8_s16_4col(kernel, buffer_a, output_ch, output_shift, output_mult,
															 out_offset, out_activation_min, out_activation_max,
															 input_ch * kernel_y * kernel_x, bias, out);
		...
		if (four_column_buf >= four_column_buf_mid + 2 * input_ch * kernel_y * kernel_x) {
			out = arm_nn_mat_mult_kernel_s8_s16(kernel, four_column_buf_mid, output_ch, output_shift, output_mult,
												out_offset, out_activation_min, out_activation_max,
												input_ch * kernel_y * kernel_x, bias, out);
