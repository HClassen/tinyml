	...
	for (c = 0; c < input_depth; c++) {
		...
		for (i = 0; i < output_height; i++) {
			for (j = 0; j < output_width - 1; j += 2) {
				...
				float sum_0 = 0.0f;
				float sum_1 = 0.0f;
				transpose_depthwise_mac_kernel7_2row_fp_uniweight(&sum_0, &sum_1, two_column_buffer, ksrc_transposed,
			...
			if (output_width & 0x1) {
				...
				// We assume bias_data as zeros.
				float sum_0 = 0.0f;
				transpose_depthwise_mac_kernel7_1row_fp_uniweight(&sum_0, two_column_buffer, ksrc_transposed,
																  input_width, STRIDE, IN_PAD, OUT_PAD);
