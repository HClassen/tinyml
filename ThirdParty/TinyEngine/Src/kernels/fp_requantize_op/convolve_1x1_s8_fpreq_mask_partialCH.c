...
tinyengine_status convolve_1x1_s8_fpreq_bitmask_partialCH(const q7_t *input, const uint16_t input_x, const uint16_t input_y, const uint16_t input_ch, const q7_t *kernel_sram,
	...
	for (int32_t i_element = 0; i_element < num_elements / 2; i_element++) {
		...
		if (first_k_channel % 4 == 0) {
			...
			out = mat_mult_kernel_s8_s16_reordered_fpreq_bitmask_partialCH(
				kernel_sram, kernel_flash, first_k_channel, two_column_buffer, output_ch, scales, (q7_t)out_offset,
				out_activation_min, out_activation_max, input_ch * DIM_KER_Y * DIM_KER_X, bias, out, mask);
		} else {
			int cnt = channel_div4; //two columns
			while (cnt > 0) {
			...
			out = mat_mult_kernel_s8_s16_reordered_fpreq_bitmask_partialCH_Multiple2(
				kernel_sram, kernel_flash, first_k_channel, two_column_buffer, output_ch, scales, (q7_t)out_offset,
				out_activation_min, out_activation_max, input_ch * DIM_KER_Y * DIM_KER_X, bias, out, mask);
		}
		mask += output_ch / 4;
...
tinyengine_status convolve_1x1_s8_fpreq_mask_partialCH(
	...
	for (i_element = 0; i_element < num_elements / 2; i_element++) {
		...
		if (first_k_channel % 4 == 0) {
			...
			out = mat_mult_kernel_s8_s16_reordered_fpreq_mask_partialCH_Multiple2(
				kernel_sram, kernel_flash, first_k_channel, two_column_buffer, output_ch, scales, (q7_t)out_offset,
				out_activation_min, out_activation_max, input_ch * DIM_KER_Y * DIM_KER_X, bias, out, mask);
		}
		mask += output_ch * 2;
